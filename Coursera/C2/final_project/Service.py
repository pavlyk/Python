...
YAML_OLD_VERSION = False

def create_sprite(img, sprite_size):
    icon = pygame.image.load(img).convert_alpha()
    icon = pygame.transform.scale(icon, (sprite_size, sprite_size))
    # sprite = pygame.Surface((sprite_size, sprite_size), pygame.HWSURFACE)
    sprite = SurfaceAdd((sprite_size, sprite_size), pygame.HWSURFACE)
    mini = pygame.Surface((5, 5), pygame.HWSURFACE)
    mini.blit(icon, (0, 0))
    sprite.mini = mini
...
class SurfaceAdd(pygame.Surface):
...
class EmptyMap(MapFactory):
    ...
    class Map:
        def __init__(self):
            # 0 - это стена
            # для пустой карты у нас стены только по краям
            self.Map = ['0'*12] + (['0' + ' '*10 + '0'])*8 + ['0'*12]
            self.Map = list(map(list, self.Map))
            # Делаем массив из спрайтов
            for i in self.Map:
                for j in range(len(i)):
                    i[j] = wall if i[j] == '0' else floor1
    ...
    class Objects:
        ...
        def get_objects(self, _map):
            ...
            # рандомное кол-во из мин и макс возможных
            for i in range(random.randint(prop['min-count'], prop['max-count'])):
                ...
                while intersect:
                    intersect = False
                    # Если координаты совпадают с стеной то продолжаем и генерируем новые
                    if _map[coord[1]][coord[0]] == wall:
                        intersect = True
                        coord = (random.randint(0, 9),
                                 random.randint(0, 9))
                        continue
                    # Такая же проверка на совпадение с любым объектом
                    for obj in self.objects:
                        if coord == obj.position or coord == (1, 1):
                            intersect = True
                            coord = (random.randint(0, 9),
                ...
                # Нашли свободные координаты
                # Добавляем в объекты новый объект (союзный) это лесница
                # его спрайт (иконку), действие и координаты
                self.objects.append(Objects.Ally(
                    prop['sprite'], prop['action'], coord))
...
def service_init(sprite_size, full=True):
    ...
    # помещаем в объекты примеры спрайта фона стены и пола
    ...
    # Для всех объектов из файла objects.yml создаем их спрайты и действия которые им присвоены
