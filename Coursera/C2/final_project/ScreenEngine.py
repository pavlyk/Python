...
class ScreenHandle(pygame.Surface):
    ...
    def __init__(self, *args, **kwargs):
        if len(args) > 1:
            self.successor = args[-1] # приемник
    ...
    def draw(self, canvas):
        if self.successor is not None:
            # Метод blit() применяется к той поверхности, на которую "накладывается", т. е. на которой "отрисовывается", другая.
            # sc.blit(surf, (50, 20))
...
class GameSurface(ScreenHandle):
        ...
        if self.game_engine.map:
            for i in range(len(self.game_engine.map[0]) - min_x):
                for j in range(len(self.game_engine.map) - min_y):
                    # берет из конца карты map[-4][-4] и накладываем на начало
    ...
    def draw(self, canvas):
        # Рисуем внутри 640х480 карту, стены, героя, объекты, лесницу и т.п.
        size = self.game_engine.sprite_size
        min_x, min_y = self.screen_offset()
...
class MiniMap(ScreenHandle):
    ...
    def draw_hero(self):
        self.game_engine.hero.draw(self, True)
        ...
        if self.game_engine.map:
            for i in range(len(self.game_engine.map[0]) - min_x):
                for j in range(len(self.game_engine.map) - min_y):
                    # берет из конца карты map[-4][-4] и накладываем на начало
                    self.blit(self.game_engine.map[min_y + j][min_x + i][
                              0].mini, (i * self.game_engine.sprite_size/20, j * self.game_engine.sprite_size/20))
        else:
            self.fill(colors["white"])

    def draw_object(self, sprite, coord):
        size = self.game_engine.sprite_size/20
        min_x, min_y = self.screen_offset()

    ##
        self.blit(sprite.mini, ((coord[0] - min_x) * self.game_engine.sprite_size/20,
                           (coord[1] - min_y) * self.game_engine.sprite_size/20))
    ...
    ##
        self.draw_map()
        for obj in self.game_engine.objects:
            self.blit(obj.sprite[0].mini, ((obj.position[0] - min_x) * self.game_engine.sprite_size/20,
                                      (obj.position[1] - min_y) * self.game_engine.sprite_size/20))
        self.draw_hero()
