...
def get_backlinks(path, end_page, unchecked_pages, checked_pages, backlinks):
    ...
    checked_pages - статьи для разузлования на текущем шаге
    unchecked_pages - все не разузлованные статьи
    backlinks - список обратных ссылок
    {'На кого ссылается': 'Кто ссылается'}
    """
    ...
    for checked_page in checked_pages:
        # удаляем из списка неразузлованных текущую статью
        unchecked_pages.remove(checked_page)
        # получаем список всех ссылок из текущей статьи которые ещё не разузлованны
        ...
        for linked_page in linked_pages:
            ...
            # добавляем в список для разузлования
...
def build_bridge(path, start_page, end_page):
    ...
    backlinks = \
        get_backlinks(path, end_page, set(
            os.listdir(path)), {start_page, }, dict())
